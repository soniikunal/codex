...
export const metadata = {
  ...
  description: "Empowering students in STEM fields with innovative learning in science",
  keywords: [
    "STEM learning",
...
export default async function ScienceWrapper() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_ADMIN_API_BASE_URL}/carousels?pageName=science`,
      {
        cache: "no-store",
      }
    );
    ...
    return <SciencePage carouselData={data} />;
  } catch (error) {
    console.error("Error fetching carousel, using fallback data:", error);
    return <SciencePage carouselData={homeCarouselBackup} />;
  }
}
