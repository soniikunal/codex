...
export default async function ScienceWrapper() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_ADMIN_API_BASE_URL}/carousels?pageName=coding`,
      {
        cache: "no-store",
      }
    );
    ...
    return <CodingPage carouselData={data} />;
  } catch (error) {
    console.error("Error fetching carousel, using fallback data:", error);
    return <CodingPage carouselData={homeCarouselBackup} />;
  }
}
